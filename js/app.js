/* ==================================================================
   ...
   Modes: founder (default) · consultant · engineer.
   ================================================================== */
(function () {
  'use strict';
  ...
  const MODES = ['founder', 'consultant', 'engineer'];
  const LEGACY = { builder: 'consultant', ai: 'engineer' };
  const THEMES = { founder: window.FX_FOUNDER, consultant: window.FX_CONSULTANT, engineer: window.FX_ENGINEER };
  const THEME_COLOR = { founder: '#0c1220', consultant: '#f4efe6', engineer: '#f5f1ea' };
  ...
  let mode = null, busy = false, triggers = [], splits = [], anims = [], storyTrigs = [], smoother = null, pendingGreet = false;

  /* ================= static rendering ================= */
  function renderStatic() {
    const items = D.marquee.concat(D.marquee);
    $('#marqueeTrack').innerHTML = items.map(t => `<span>${esc(t)}</span>`).join('');
    $('#principlesList').innerHTML = D.principles.map(p => `<div class="prow" data-reveal><span class="n">${p.n}</span><h3>${esc(p.title)}</h3><p>${esc(p.desc)}</p></div>`).join('');
    $('#timeline').innerHTML = D.experience.map(x => `
      <div class="tl-item" data-reveal>
        ...
        <div><h3 class="tl-role">${esc(x.role)}</h3><p class="tl-org">${esc(x.org)}</p><ul>${x.points.map(p => `<li>${esc(p)}</li>`).join('')}</ul></div>
      </div>`).join('');
    ...
    $('#eduGrid').innerHTML = D.education.map(e => `<article class="card edu" data-reveal><h3>${esc(e.title)}</h3><p class="org">${esc(e.org)}</p>${e.desc ? `<p class="desc">${esc(e.desc)}</p>` : ''}<span class="when">${esc(e.status)}</span></article>`).join('');
    $('#recGrid').innerHTML = D.recognition.map(r => `<article class="card rec" data-reveal><h3>${esc(r.title)}</h3><p>${esc(r.desc)}</p>${r.link ? `<a class="arrow-link" href="${r.link}"${ext(r.link)}>${esc(r.linkText)}</a>` : ''}</article>`).join('');
  ...
  function applyCopy() {
    ...
    $('#stats').innerHTML = C.stats.map(s => `<div class="stat" data-reveal><div class="stat-v" data-count="${s.v}" data-dec="${s.dec || 0}" data-pre="${esc(s.pre || '')}" data-suf="${esc(s.suf || '')}">${esc((s.pre || '') + '0' + (s.suf || ''))}</div><div class="stat-l">${esc(s.l)}</div></div>`).join('');
    ...
    const tc = $('#termsClause'); if (tc) tc.textContent = 'Your terms appear here as you type.';
  }

  /* ================= reveals ================= */
  const RECIPES = {
    founder: { from: { y: 40, opacity: 0 }, to: { y: 0, opacity: 1, duration: 1.1, ease: 'power3.out' } },
    consultant: { from: { y: 28, opacity: 0 }, to: { y: 0, opacity: 1, duration: 1.05, ease: 'power3.out' } },
  ...
  function teardownReveals() {
    ...
    /* revert unplayed reveal tweens so nothing below the fold stays stuck in its hidden state */
    anims.forEach(a => { try { a.revert(); } catch (e) { try { a.kill(); } catch (e2) { /* ignore */ } } }); anims = [];
    splits.forEach(s => { try { s.revert(); } catch (e) { /* ignore */ } }); splits = [];
    if (hasGsap) gsap.set($$('.section-head > *, .hero-copy > *'), { clearProps: 'all' });
  }
  function setupReveals() {
    ...
    els.forEach(el => {
      ...
      const delay = Math.min(Math.max(0, sibs.indexOf(el)), 6) * 0.07;
      ...
      if (hasSplit && heads.length) {
        ...
        heads.forEach(h => {
          try { const sp = SplitText.create(h, { type: 'lines', mask: 'lines', linesClass: 'sline' }); splits.push(sp); anim.from(sp.lines, { yPercent: 115, duration: 1.05, ease: 'power4.out', stagger: 0.09 }, 0); }
      ...
      anims.push(anim);
      triggers.push(ScrollTrigger.create({ trigger: el, start: 'top 92%', once: true, onEnter: () => anim.play() }));
    });
  }
  function countUp(el) {
  ...
  const TR = {
    founder(swap) {
      return new Promise(res => {
        ...
        gsap.set(I, { visibility: 'visible', clipPath: 'circle(0% at 50% 50%)' }); gsap.set(Bt, { scale: 0.3, rotation: -360, opacity: 0 });
        gsap.timeline({ onComplete: () => { gsap.set([I, Hh], { visibility: 'hidden' }); gsap.set(Hh, { width: 0, height: 0 }); res(); } })
          .to(I, { clipPath: 'circle(75% at 50% 50%)', duration: 0.45, ease: 'power2.in' })
          .to(Bt, { scale: 1, rotation: 0, opacity: 1, duration: 0.5, ease: 'power3.out' }, '<0.1')
          .add(swap, '+=0.02')
          .to(Bt, { scale: 1.5, opacity: 0, duration: 0.3, ease: 'power2.in' }, '+=0.15')
          .set(Hh, { visibility: 'visible', width: 0, height: 0 }).set(I, { visibility: 'hidden' })
          .to(Hh, { width: '320vmax', height: '320vmax', duration: 0.75, ease: 'power2.inOut' });
      });
    },
    consultant(swap) {
      return new Promise(res => {
        const A = $('.trans-door-l'), B = $('.trans-door-r'), L = $('.trans-door-label');
        gsap.set([A, B], { visibility: 'visible' }); gsap.set(A, { xPercent: -100 }); gsap.set(B, { xPercent: 100 }); gsap.set(L, { visibility: 'visible', opacity: 0, y: 8 });
        gsap.timeline({ onComplete: () => { gsap.set([A, B, L], { visibility: 'hidden' }); res(); } })
          .to([A, B], { xPercent: 0, duration: 0.45, ease: 'power3.inOut' })
          .to(L, { opacity: 1, y: 0, duration: 0.3 }, '-=0.1')
          .add(swap, '+=0.02')
          .to(L, { opacity: 0, duration: 0.2 }, '+=0.25')
          .to(A, { xPercent: -100, duration: 0.55, ease: 'power3.inOut' }, '<')
          .to(B, { xPercent: 100, duration: 0.55, ease: 'power3.inOut' }, '<');
      });
    },
    engineer(swap) {
      return new Promise(res => {
        ...
        gsap.timeline({ onComplete: () => { gsap.set(G, { visibility: 'hidden' }); res(); } })
          .to(G, { opacity: 1, duration: 0.25, ease: 'power2.out' })
          .to(ring, { scale: 1, rotation: 0, opacity: 1, duration: 0.5, ease: 'power4.out' }, '<0.05')
          .to(label, { opacity: 1, duration: 0.2 }, '<0.25')
          .add(swap, '+=0.02')
          .to(ring, { scale: 1.15, opacity: 0, duration: 0.3, ease: 'power2.in' }, '+=0.25')
          .to(G, { opacity: 0, duration: 0.45, ease: 'power2.inOut' }, '<0.1');
      });
    }
  };
  ...
  function applyMode(next) {
    ...
    updateToggle(next); applyCopy();
    const T = THEMES[next];
    if (T) {
      T.mount({
        bg: $('#bg'), fx: $('#fx'), hero: $('#top'), stats: $('#stats'), reduce,
        stage: next === 'engineer' ? $('#reactorCore') : null,
        suit: $('#suit'), jarvis: $('#jarvisLine'), greeting: D.jarvis, hudVals: $$('#suit [data-base]'),
        hud: { hint: $('#reactorHint') },
        deal: $('#deal'), deals: D.deals
      });
      if (T.contactMount) T.contactMount($('#stage'), $$('.clink'), D);
      if (next === 'engineer' && T.greet) {
        if (pendingGreet) setTimeout(() => T.greet(), 900);
        else { const once = () => { document.removeEventListener('pointerdown', once); document.removeEventListener('keydown', once); if (mode === 'engineer') T.greet(); }; document.addEventListener('pointerdown', once); document.addEventListener('keydown', once); }
      }
    }
  ...
  function updateToggle(m) {
    $('.mode-pill').style.setProperty('--i', MODES.indexOf(m));
  ...
  function buttonFx(btn, e) {
    ...
    if (mode === 'consultant') { retrigger(btn, 'inked'); ripple(btn, x, y); }
  ...
  function initNav() {
    ...
    $('#modeToggle').addEventListener('click', e => { const b = e.target.closest('button[data-mode]'); if (b) switchMode(b.dataset.mode, { gesture: true }); });
    ...
    document.addEventListener('keydown', e => {
      ...
      if (e.key === '1') switchMode('founder', { gesture: true }); else if (e.key === '2') switchMode('consultant', { gesture: true }); else if (e.key === '3') switchMode('engineer', { gesture: true });
    });
  }
  function initSound() {
    ...
    set(false);
    btn.addEventListener('click', () => { if (!window.SOUND) return; const on = !window.SOUND.isEnabled(); window.SOUND.setEnabled(on); window.SOUND.setVoiceMuted(false); set(on); if (on) window.SOUND.play('click'); });
  }
  ...
  function initForm() {
    const msg = $('#fmsg'), tc = $('#termsClause');
    msg.addEventListener('input', () => { if (!tc) return; const v = msg.value.trim(); tc.textContent = v ? (v.length > 160 ? v.slice(0, 160) + '…' : v) : 'Your terms appear here as you type.'; });
    $('#cform').addEventListener('submit', e => {
      ...
      toast({ founder: 'Signal sent. Opening your mail app.', consultant: 'Terms received. Opening your mail app.', engineer: 'Transmitting. Opening your mail app.' }[mode]);
  ...
  function init() {
    ...
    if (document.fonts && document.fonts.load) {
      const faces = ['400 1em Inter', '600 1em Inter', '400 1em "Bebas Neue"', '600 1em "Playfair Display"', 'italic 600 1em "Playfair Display"', '700 1em "Space Grotesk"', '400 1em "JetBrains Mono"'];
